...
// La parte laboral de los PROFESIONALES: su expediente, sus ausencias y el
// registro de jornada. Son quienes hacen los servicios en casa de las
// personas, trabajen para la empresa o por su cuenta.
//
...
export function PersonalTab() {
  ...
  const [expediente, setExpediente] = useState<FichaProfesional | null>(null);
  ...
  async function cargar() {
    const [e, a, r] = await Promise.all([
      api.get<FichaProfesional[]>("/personal", token),
  ...
  const visibles = useMemo(
    () => plantilla.filter((m) => (!q ? true : `${m.nombre} ${m.apellidos} ${m.codigo}`.toLowerCase().includes(q))),
    [plantilla, q],
  );

  const bloqueados = plantilla.filter((m) => m.bloqueado);
  const porRenovar = plantilla.filter((m) => !m.bloqueado && m.carencias.length > 0);
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-1.5">
        {([
          ["plantilla", "Plantilla", plantilla.length],
...
function ExpedienteModal({
  ...
  async function registrarAusencia() {
    ...
    onError(null);
    try {
      await api.post(`/personal/${miembro.id}/ausencias`, nuevaAusencia, token);
